...
Core={
	...
	Settings={
		...
		Owner=game:GetService("Players"):FindFirstChild("CoreSB"); --//Your name here!
	};
	Ranks={
		[1]="Guest";
		[2]="User";
...
function ConnectPlayer(Player)
	...
	if Core.Players[Player.Name] == nil then
		...
		AddPart("[Core]\nVersion 1.5",'Really red',Player,nil);
	else
		Player.Chatted:connect(function(Message)
			OnChatted(Message,Player);
		end);
...
NewCommand("Remove/Stop/Kill","Stops Core",7,{"remove","nos"},{},function(Message,Flags,Player,PlayerRank)
	pcall(function()
		AddPart("[Core]\nCore is shutting down, please wait...","Bright green",Player,nil)
		wait(3);
		script:Destroy();
	end);
end);
...
NewCommand("Execute","Executes/Compiles Lua Code",1,{"exe"},{"http"},function(Message,Flags,Player,PlayerRank)
	...
	Execute,Error=loadstring(tostring(Source),"[Core Execute]");
...
for i,v in pairs(game:GetService("Players"):GetPlayers()) do
	...
	if Core.Cloud ~= nil then	
		AddPart("The Cloud is active","Bright blue",v,nil);				
	end
end;
